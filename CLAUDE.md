...
- Paths NOT matched by `.rp/shadow` pass through `rp-fuse` to the host bind (reached via the captured fd). Edits to source files propagate to the host as expected.
