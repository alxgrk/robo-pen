...
for _ in $(seq 1 30); do
    container exec -u 0 "$cont" awk '$2=="/workspace" && $3 ~ /^fuse/' /proc/mounts 2>/dev/null | grep -q . && break
    sleep 0.2
done

mounts=$(container exec -u 0 "$cont" awk '$2=="/workspace" {print $3}' /proc/mounts 2>&1)
...
[ "$count" -ge 3 ] || fail "expected >=3 mount lines at /workspace, got $count: $mounts"

top=$(echo "$mounts" | tail -1)
mid=$(echo "$mounts" | tail -2 | head -1)

echo "$top" | grep -q '^fuse' || fail "topmost mount at /workspace is not fuse, got '$top' (full: $mounts)"
[ "$mid" = "tmpfs" ] || fail "middle mount at /workspace is not tmpfs, got '$mid' (full: $mounts)"
